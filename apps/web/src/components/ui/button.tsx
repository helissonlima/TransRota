...
const variantStyles: Record<ButtonVariant, string> = {
  ...
  ghost:
    'bg-transparent text-brand-text-secondary hover:bg-slate-100 hover:text-brand-text-primary',
  danger:
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    return (
      ...
      >
        {loading ? (
          <Loader2 className={cn('animate-spin flex-shrink-0', size === 'xs' ? 'w-3 h-3' : 'w-4 h-4')} />
        ) : (
