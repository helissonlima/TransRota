...
export function Header({
  ...
  return (
    <header className="sticky top-0 z-10 bg-white/90 backdrop-blur-md border-b border-brand-border/40 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center px-4 md:px-6 py-3 md:py-0 md:h-16 gap-3 md:gap-4">
      ...
      <div className="flex items-center gap-3 min-w-0">
          ...
          >
            {breadcrumbLinks.map((crumb, idx) => (
              <span key={idx} className="flex items-center gap-1.5 min-w-0">
                {idx > 0 && (
                  <ChevronRight className="w-3.5 h-3.5 text-brand-text-secondary flex-shrink-0" />
                )}
                <Link
        ...
        <AnimatePresence>
          {selectedItemLabel && (onEditSelected || onDeleteSelected) && (
            ...
            >
              <span className="text-sm text-brand-text-secondary mr-1 hidden md:inline">
                {selectedItemLabel}
              </span>
              {onEditSelected && (
                <button
      ...
      <div className="min-w-0 max-w-full md:max-w-[40vw] order-first md:order-none">
        <h1 className="text-lg md:text-xl font-bold text-brand-text-primary truncate text-left md:text-center">
          {title}
        </h1>
      </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-brand-text-secondary mt-0.5 line-clamp-2">
                              {n.description}
                            </p>
                          </div>
                          <span className="text-2xs text-brand-text-secondary flex-shrink-0 mt-1">
                            {n.time}
                          </span>
                        </div>
                      );
