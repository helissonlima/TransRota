...
export default function ProductsPage() {
  ...
  const advanceOrder = useMutation({
    mutationFn: ({ id, action }: { id: string; action: string }) =>
      api.patch(`/production-orders/${id}/${action}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["production-orders"] });
      toast.success("Ordem atualizada");
    },
    onError: (e: any) =>
      toast.error(e.response?.data?.message ?? "Erro ao avançar ordem"),
  });

  // ──── Forms ────
  const productForm = useForm<ProductForm>({
    resolver: zodResolver(productSchema),
    defaultValues: { type: "FINISHED_GOOD", unit: "UN" },
  });
  const movForm = useForm<MovementForm>({
    resolver: zodResolver(movementSchema),
    defaultValues: { type: "ENTRY" },
  });
  const prodForm = useForm<ProdOrderForm>({
    resolver: zodResolver(prodOrderSchema),
  });
    ...
    <div className="flex flex-col min-h-screen bg-brand-bg">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {alerts.length > 0 && (
            <span className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-amber-50 text-amber-700 border border-amber-200">
              <AlertTriangle className="w-3.5 h-3.5" /> {alerts.length} alerta
              {alerts.length !== 1 ? "s" : ""} de estoque
            </span>
          )}
        </div>
          ...
          <div>
            {tab === "catalog" && (
              <button
                ...
                className="flex items-center gap-2 text-sm px-4 py-2 rounded-xl bg-primary-600 hover:bg-primary-700 text-white transition-colors"
              >
        ...
        <div>
            ...
            <div>
              <div className="relative mb-5">
                ...
                <input
                  ...
                  className="w-full max-w-md bg-white border border-brand-border rounded-xl pl-10 pr-4 py-2.5 text-sm text-brand-text-primary placeholder:text-brand-text-secondary focus:outline-none focus:border-primary-500"
                />
              </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                <div className="bg-white border border-brand-border rounded-2xl p-4 shadow-card">
                  <p className="text-xs text-brand-text-secondary uppercase tracking-wide mb-1">
                    Total de Itens
                  </p>
                  ...
                  <p className="text-2xl font-bold text-emerald-600">
                    {stock
                      .reduce(
                        (sum, s) =>
                          sum +
                          Number(s.quantity) * Number(s.product.costPrice),
                        0,
                      )
                ...
                <div className="bg-white border border-brand-border rounded-2xl p-4 shadow-card">
                  ...
                  <p className="text-2xl font-bold text-blue-600">
                    {
                      new Set(stock.map((s) => s.location?.name ?? "Principal"))
                        .size
                    }
                  </p>
                </div>
                ...
                <div className="mb-5 p-4 rounded-2xl bg-amber-50 border border-amber-200">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {alerts.map((a) => (
                      <span
                        key={a.id}
                        className="text-xs px-3 py-1 rounded-full bg-amber-100 text-amber-700 border border-amber-200"
                      >
                        {a.product.name} —{" "}
                        {Number(a.quantity).toLocaleString("pt-BR", {
                          maximumFractionDigits: 3,
                        })}{" "}
                        {a.product.unit}
                      </span>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {stock.map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5 text-brand-text-secondary font-mono text-xs">
                            {s.product.sku}
                          </td>
                          <td className="px-5 py-3.5">
                            <span
                              className={cn(
                                ...
                                TYPE_COLOR[s.product.type],
                              )}
                            >
                              {TYPE_LABEL[s.product.type]}
                            </span>
                          </td>
                          <td
                            className={cn(
                          ...
                          <td className="px-5 py-3.5 text-brand-text-secondary">
                            {s.product.minStock
                              ? `${Number(s.product.minStock)} ${s.product.unit}`
                              : "—"}
                          </td>
                          <td className="px-5 py-3.5 text-brand-text-secondary">
                            {s.location?.name ?? "Principal"}
                          </td>
                          <td className="px-5 py-3.5">
                            {isLow ? (
                    ...
                    {stock.length === 0 && (
                      <tr>
                        <td
                          colSpan={7}
                          className="text-center py-16 text-brand-text-secondary"
                        >
                ...
                <div className="space-y-5">
                  {supplierWarehouseRows.map(({ supplier, rows }) => {
                    return (
                      <div
                        key={supplier.id}
                        className="rounded-2xl border border-brand-border overflow-hidden shadow-card"
                      >
                        <div
                          className={cn(
                            "px-5 py-3 border-b border-brand-border font-semibold text-sm",
                            supplier.color
                              ? `text-white ${supplier.color}`
                              : "bg-slate-100 text-brand-text-primary",
                          )}
                        >
                          {supplier.name}
                        </div>
                      ...
                      {supplierWarehouseRows.flatMap(({ supplier, rows }) => {
                        ...
                        return [
                          ...
                          <tr
                            ...
                            className="bg-slate-50 border-t-2 border-slate-200 font-semibold"
                          >
                            <td
                              colSpan={2}
                              className="px-5 py-3.5 text-brand-text-primary"
                            >
          ...
          {tab === "bom" && (
            <div className="max-w-3xl">
              <div className="mb-5">
                <label className="block text-xs text-brand-text-secondary mb-2">
                  Selecione o produto para ver/editar a ficha técnica
                </label>
                <select
                  value={bomProductId}
                ...
                >
                  ...
                  {products
                    .filter(
                      (p) => p.type !== "RAW_MATERIAL" && p.type !== "SERVICE",
                    )
                    .map((p) => (
              ...
              {bomProductId && !bomLoading && bom && (
                <div className="rounded-2xl border border-brand-border shadow-card overflow-hidden">
                  <div className="bg-slate-50 px-5 py-4 border-b border-brand-border">
                    <p className="font-semibold text-sm text-brand-text-primary">
                      Ficha Técnica —{" "}
                      {products.find((p) => p.id === bomProductId)?.name}
                    </p>
                    <p className="text-xs text-brand-text-secondary mt-0.5">
                      Rendimento por ciclo: {Number(bom.yield)}{" "}
                      {products.find((p) => p.id === bomProductId)?.unit}
                    </p>
                  </div>
                  <table className="w-full text-sm">
              ...
              {bomProductId && !bomLoading && !bom && (
                <div className="text-center py-12 text-brand-text-secondary">
                  ...
                  <p className="text-xs mt-1">
                    Use a API para criar a BOM via POST /products/:id/bom
                  </p>
                </div>
              )}
            </div>
                ...
                <tbody className="divide-y divide-gray-100">
                  {productionOrders.map((o) => (
                    <tr
                      ...
                      className="hover:bg-slate-50 transition-colors"
                    >
                      <td className="px-5 py-3.5 font-mono text-xs text-brand-text-secondary">
                        {o.number}
                      </td>
                      <td className="px-5 py-3.5 font-medium">
                        {o.product.name}
                      ...
                      <td className="px-5 py-3.5">
                        {Number(o.quantity)} {o.product.unit}
                      </td>
                      <td className="px-5 py-3.5">
                        <span
                          className={cn(
                        ...
                        <div className="flex items-center gap-2">
                          {o.status === "DRAFT" && (
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded-lg text-blue-400 hover:bg-blue-900/20 transition-colors"
                            >
                              Iniciar
                            </button>
                          )}
                          {o.status === "IN_PROGRESS" && (
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded-lg text-emerald-400 hover:bg-emerald-900/20 transition-colors"
                            >
                              Concluir
                            </button>
                          )}
                          {(o.status === "DRAFT" ||
                            ...
                            <button
                              ...
                              className="text-xs px-2.5 py-1 rounded-lg text-red-400 hover:bg-red-900/20 transition-colors"
                            >
                              Cancelar
                            </button>
                          )}
          ...
          {tab === "movements" && (
            <div className="rounded-2xl border border-brand-border shadow-card overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-slate-50 text-brand-text-secondary text-xs uppercase tracking-wide">
                  <tr>
                    {["Produto", "Tipo", "Quantidade", "Motivo", "Data"].map(
                      (h) => (
                        <th key={h} className="px-5 py-3 text-left font-medium">
                          {h}
                        </th>
            ...
            >
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  className="px-5 py-2 text-sm bg-primary-600 hover:bg-primary-700 text-white rounded-xl transition-colors"
                >
      ...
      <AnimatePresence>
        {showMovModal && (
          ...
          >
            <form
              onSubmit={movForm.handleSubmit((d) => createMovement.mutate(d))}
              className="space-y-4"
            >
              <Field
                label="Produto"
                error={movForm.formState.errors.productId?.message}
              >
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button
                  ...
                  className="px-4 py-2 text-sm text-brand-text-secondary"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="px-5 py-2 text-sm bg-primary-600 hover:bg-primary-700 text-white rounded-xl"
                >
      ...
      <AnimatePresence>
        {showProdModal && (
          ...
          >
            <form
              onSubmit={prodForm.handleSubmit((d) => createProdOrder.mutate(d))}
              className="space-y-4"
            >
              <Field
                label="Produto a produzir"
                error={prodForm.formState.errors.productId?.message}
              >
                <select
                  {...prodForm.register("productId")}
                  className={inputCls}
                >
                  <option value="">— Selecione —</option>
                  {products
              ...
              <Field
                label="Quantidade"
                error={prodForm.formState.errors.quantity?.message}
              >
                <input
                  {...prodForm.register("quantity")}
                  type="number"
                  step="0.001"
                  placeholder="0"
                  className={inputCls}
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  disabled={createProdOrder.isPending}
                  className="px-5 py-2 text-sm bg-primary-600 hover:bg-primary-700 text-white rounded-xl"
                >
...
// ───────────────────────────────────────────────────────────────
const inputCls =
  "w-full bg-slate-50 border border-brand-border rounded-xl px-3.5 py-2.5 text-sm text-brand-text-primary placeholder:text-brand-text-secondary focus:outline-none focus:border-primary-500";
    ...
    >
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-brand-border">
          <h2 className="font-semibold text-sm text-brand-text-primary">
            {title}
          </h2>
          <button
            onClick={onClose}
