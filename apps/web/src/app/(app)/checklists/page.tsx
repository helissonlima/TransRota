...
export default function ChecklistsPage() {
  ...
  const [selectedExecution, setSelectedExecution] =
    useState<ChecklistExecution | null>(null);
  ...
  >({
    ...
    queryFn: () =>
      api
        .get("/checklists", { params: { type: typeFilter || undefined } })
        .then((r) => r.data),
  });
  ...
  >({
    ...
    queryFn: () =>
      api
        .get("/checklists/executions", { params: { limit: 20 } })
        .then((r) => r.data),
  });
  ...
  const resolveMutation = useMutation({
    mutationFn: ({
      ...
      api
        .patch(`/checklists/executions/${id}/resolve`, { status })
        .then((r) => r.data),
    onSuccess: (updated) => {
      toast.success(
        updated.resolutionStatus === "APPROVED"
          ? "Execução aprovada!"
          : "Execução marcada como resolvida!",
      );
      qc.invalidateQueries({ queryKey: ["checklist-executions"] });
  ...
  const executionColumns: Column<ChecklistExecution>[] = [
    {
      ...
      cell: (row) => (
        <span className="text-xs text-brand-text-secondary">
          {row.createdAt
            ? format(parseISO(row.createdAt), "dd/MM 'às' HH:mm", {
                locale: ptBR,
              })
            : "—"}
        </span>
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="flex flex-col">
          <span className="font-plate font-bold text-sm text-brand-text-primary">
            {row.vehicle.plate}
          </span>
          <span className="text-xs text-brand-text-secondary">
            {row.vehicle.brand}
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6 space-y-6 max-w-[1600px] mx-auto">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            {
              label: "Templates",
              value: checklistStats.templates,
              icon: ClipboardList,
              tone: "bg-blue-50 text-blue-700 border-blue-100",
            },
            {
              label: "Execuções",
              value: checklistStats.executions,
              icon: ClipboardCheck,
              tone: "bg-emerald-50 text-emerald-700 border-emerald-100",
            },
            {
              label: "Com Problemas",
              value: checklistStats.withIssues,
              icon: AlertCircle,
              tone: "bg-amber-50 text-amber-700 border-amber-100",
            },
            {
              label: "Pendentes",
              value: checklistStats.pending,
              icon: XCircle,
            ...
            <div className="flex items-center bg-white border border-brand-border rounded-xl p-1 gap-0.5">
              ...
              {(
                Object.entries(TYPE_CONFIG) as [
                  ChecklistType,
                  (typeof TYPE_CONFIG)[ChecklistType],
                ][]
              ).map(([key, cfg]) => (
                <button
                  key={key}
                      ...
                      <div className="p-5">
                        ...
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-semibold text-brand-text-primary text-sm leading-tight">
                                {template.name}
                              </h3>
                        ...
                        <div className="space-y-1 mb-4">
                          {template.items.slice(0, 3).map((item, j) => (
                            ...
                            >
                              ...
                              {item.required && (
                                <span className="text-danger-400 flex-shrink-0">
                                  *
                                </span>
                              )}
                            </div>
      ...
      {selectedExecution && (
        <DetailPanel
          ...
          subtitle={`${selectedExecution.createdAt ? format(parseISO(selectedExecution.createdAt), "dd 'de' MMMM 'de' yyyy 'às' HH:mm", { locale: ptBR }) : ""} • ${selectedExecution.driver.name}`}
          badges={[
            {
              label: TYPE_CONFIG[selectedExecution.checklist.type].label,
              variant: TYPE_CONFIG[selectedExecution.checklist.type]
                .variant as any,
            },
            (() => {
              if (selectedExecution.resolutionStatus !== "PENDING") {
                const config =
                  RESOLUTION_CONFIG[selectedExecution.resolutionStatus];
                return { label: config.label, variant: config.variant as any };
              }
              return selectedExecution.hasIssues
                ? { label: "Com Problemas", variant: "danger" as const }
        ...
        >
          <DetailPanel.Section title="Informações">
            <DetailPanel.Grid cols={2}>
                ...
                value={
                  <>
                    <span className="font-mono">
                      {selectedExecution.vehicle.plate}
                    </span>{" "}
                    — {selectedExecution.vehicle.brand}{" "}
                    {selectedExecution.vehicle.model}
                  </>
                }
              />
              <DetailPanel.Field
                label="Motorista"
                value={selectedExecution.driver.name}
              />
              {selectedExecution.inspectorName && (
                <DetailPanel.Field
                  label="Inspetor"
                  value={selectedExecution.inspectorName}
                />
              )}
              {selectedExecution.fuelLevel && (
                <DetailPanel.Field
                  label="Combustível"
                  value={selectedExecution.fuelLevel}
                />
              )}
              {selectedExecution.unitLocation && (
                <DetailPanel.Field
                  label="Localização"
                  value={selectedExecution.unitLocation}
                />
              )}
              <DetailPanel.Field
                label="Data/Hora"
                value={format(
                  parseISO(selectedExecution.createdAt),
                  "dd/MM/yyyy 'às' HH:mm",
                  { locale: ptBR },
                )}
              />
              {selectedExecution.resolvedAt && (
                <DetailPanel.Field
                  label={
                    selectedExecution.resolutionStatus === "APPROVED"
                      ? "Aprovado em"
                      : "Resolvido em"
                  }
                  value={`${format(parseISO(selectedExecution.resolvedAt), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })}${selectedExecution.resolvedByName ? ` por ${selectedExecution.resolvedByName}` : ""}`}
                />
              )}
            </DetailPanel.Grid>
          </DetailPanel.Section>
          ...
          {(selectedExecution.externalDamage ||
            ...
            <DetailPanel.Section title="Avarias">
          ...
          {selectedExecution.responses &&
            selectedExecution.responses.length > 0 && (
              <DetailPanel.Section title="Itens Verificados">
                  ...
                  {selectedExecution.responses.map((r) => (
                    <div
                      ...
                      className={cn(
                        ...
                        r.status === "NOK" && "bg-red-50 border-red-200",
          ...
          {selectedExecution.resolutionStatus === "PENDING" &&
            selectedExecution.hasIssues && (
              <div className="pt-2">
                <Button
                  className="w-full"
                  variant="secondary"
        ...
        <form className="space-y-5">
          ...
          <div>
            ...
            {errors.items && !errors.items[0] && (
              <p className="text-danger-500 text-xs mb-2">
              ...
              <AnimatePresence>
                {fields.map((field, index) => (
                  ...
                  >
                    ...
                    <span className="text-xs font-bold text-brand-text-secondary w-5 flex-shrink-0">
                      {index + 1}
                    </span>
                    <input
                      ...
                      className={cn(
                        "flex-1 text-sm bg-transparent outline-none text-brand-text-primary placeholder:text-brand-text-secondary/50 min-w-0",
                        errors.items?.[index]?.description && "text-danger-600",
                      )}
                    />
      ...
      {selectedTemplate && (
        <DetailPanel
          ...
          badges={[
            {
              label: TYPE_CONFIG[selectedTemplate.type].label,
              variant: TYPE_CONFIG[selectedTemplate.type].variant as any,
            },
          ]}
          width="md"
        >
          <DetailPanel.Section title="Resumo">
            <DetailPanel.Grid cols={2}>
              <DetailPanel.Field
                label="Itens"
                value={selectedTemplate.items.length}
              />
              <DetailPanel.Field
                label="Execuções"
                value={selectedTemplate._count.executions}
              />
            </DetailPanel.Grid>
          </DetailPanel.Section>

          <DetailPanel.Section title="Itens de Verificação">
            <div className="space-y-1.5">
              {selectedTemplate.items.map((item, i) => (
                <div
                  key={i}
                  className="flex items-center gap-3 p-3 bg-slate-50 rounded-xl border border-slate-200/50"
                >
                  ...
                  {item.required && (
                    <span className="text-2xs text-red-500 font-semibold bg-red-50 px-1.5 py-0.5 rounded-md flex-shrink-0">
                      Obrigatório
                    </span>
                  )}
                </div>
          ...
          <form className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  {...registerExecute("vehicleId")}
                  className={cn(
                    "input-base",
                    executeErrors.vehicleId && "border-danger-400",
                  )}
                >
                ...
                {executeErrors.vehicleId && (
                  <p className="text-danger-500 text-xs mt-1">
                    {executeErrors.vehicleId.message}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <select
                  {...registerExecute("driverId")}
                  className={cn(
                    "input-base",
                    executeErrors.driverId && "border-danger-400",
                  )}
                >
                ...
                {executeErrors.driverId && (
                  <p className="text-danger-500 text-xs mt-1">
                    {executeErrors.driverId.message}
                  </p>
                )}
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold text-brand-text-primary mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <User className="w-3.5 h-3.5" /> Inspetor
                  </span>
                </label>
                <input
              ...
              <div>
                <label className="block text-sm font-semibold text-brand-text-primary mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <Fuel className="w-3.5 h-3.5" /> Nível de Combustível
                  </span>
                </label>
                <select
                  {...registerExecute("fuelLevel")}
                  className="input-base"
                >
              ...
              <div>
                <label className="block text-sm font-semibold text-brand-text-primary mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5" /> Localização da Unidade
                  </span>
                </label>
                <input
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-brand-text-primary mb-1.5">
                  <span className="flex items-center gap-1.5">
                    <Shield className="w-3.5 h-3.5 text-warning-500" /> Avaria
                    Externa
                  </span>
                </label>
                <textarea
